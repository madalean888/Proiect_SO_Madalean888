...
void create(const char *district_id){
    ...
    if(symlink(path,link)==-1){
        perror("Warning: Error while creating symbolic link!\n");
        return;
    }
}
...
int log_action(const char* district_id, char* role, char* user, char* action){
    ...
    if (strcmp(role, "inspector") == 0) {
        printf("Warning: Inspectors cannot write to the log file!\n");
        return -1;
    }
    char path[512];
    ...
    if(loggedDistrict==-1){
        perror("Warning: Error while opening logged_district!");
        return -1;
    }
    ...
    if(role==NULL || user==NULL){
        perror("Warning: Error while passing details for the log!");
        return -1;
    }
...
int add(const char* district_id, char* user, char* role){
    ...
    if(reports==-1){
        perror("Warning: Error while opening reports.dat!\n");
        closedir(d);
        return -1;
    }
    ...
    if(write(reports,&r,sizeof(r))==-1){
        perror("Warning: Error while writing the report!\n");
        close(reports);
        closedir(d);
        return -1;
    }
    ...
    int monitor=open(".monitor_pid",O_RDONLY);

    int signal_verification=0;

    if(monitor==-1){
        printf("Warning: Error while opening .monitor_pid!\n");
    }
    else{
        char string_pid[10];
        memset(string_pid,0,sizeof(string_pid));

        if(read(monitor,string_pid,sizeof(string_pid)-1)<1){
            printf("The .monitor_pid file is empty!\n");
        }
        else{
            pid_t monitor_pid=atoi(string_pid);

            if(kill(monitor_pid,SIGUSR1)==0){
                signal_verification=1;
            }
            else{
                printf("Warning: The signal could not be sent!\n");
            }
        }
        close(monitor);
    }
    ...
    char action[256];
    if(signal_verification==1){
        snprintf(action,sizeof(action),"Added: Report No. %d | Monitor was successfully notified!\n",r.ReportID);
    }else{
        snprintf(action,sizeof(action),"Added: Report No. %d | Warning: Monitor could not be notified!\n",r.ReportID);
    }
...
int remove_report(const char* district_id,int report_id, char* user, char* role){
    ...
    if(!found){
        ...
        if(ftruncate(reports,write_pos)==-1){
            printf("Warning: Error while truncating reports.dat!\n");
            close(reports);
            return -1;
        }
        else{
...
int update_threshold(const char* district_id, int value, char* user, char* role){
    ...
    if ((st.st_mode & 0777) != 0640) {
        printf("Warning: district.cfg permissions are incorrect (%o). Aborting for security!\n", st.st_mode & 0777);
        return -1;
    }
    int districtCFG=open(path,O_WRONLY | O_TRUNC);

    if(districtCFG==-1){
        perror("Warning: Error while opening district.cfg!\n");
        return -1;
    }
...
int filter(const char* district_id, char* condition){
    ...
    if(parse_condition(condition,type,operation,value)){
        perror("Warning: Error while parsing the condition!\n");
        return -1;
    }
...
int remove_district(const char* district_id){
    ...
    if((p=fork())<0){
        perror("Warning: Error while using the fork function!\n");
        return -1;
    }
    else if(p==0){
        ...
        if(unlink(link)==-1){
            perror("Warning: Error while trying to delete the symbolic link\n");
            return -1;
        }
        else{
            printf("The symbolic link was deleted successfully!\n");
        }

        execlp ("rm","rm","-rf",district_id,NULL);

        perror("Warning: Error executing the removal of the district!\n");
    ...
    if(command!=NULL){
        ...
        if(strcmp(command,"remove_district")==0){
            if(role_level==1){
                ...
                printf("The remove_district operation can not be done by someone who is not the manager!");
            }
        }
    }
    return 0;
